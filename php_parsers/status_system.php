...
if (isset($_POST['action']) && $_POST['action'] == "status_post"){
	...
	mysqli_query($connection, "UPDATE status SET osid='$id' WHERE id='$id' LIMIT 1");
	...
	if ($row[0] > 9) { // If they have 10 or more posts of type a
		...
		$row = mysqli_fetch_row($query);
		$oldest = $row[0];
		mysqli_query($connection, "DELETE FROM status WHERE osid='$oldest'");
	}
	// Insert notifications to all friends of the post author
	$friends = array();
	$query = mysqli_query($connection, "SELECT user1 FROM friends WHERE user2='$log_username' AND accepted='1'");
	while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) { array_push($friends, $row["user1"]); }
	$query = mysqli_query($connection, "SELECT user2 FROM friends WHERE user1='$log_username' AND accepted='1'");
	...
	for($i = 0; $i < count($friends); $i++){
		...
		mysqli_query($connection, "INSERT INTO notifications(username, initiator, app, note, date_time) VALUES('$friend','$log_username','$app','$note',now())");
	}
	mysqli_close($connection);
	echo "post_ok|$id";
	exit();
...
if (isset($_POST['action']) && $_POST['action'] == "status_reply"){
	...
	while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
		...
		mysqli_query($connection, "INSERT INTO notifications(username, initiator, app, note, date_time)
...
if (isset($_POST['action']) && $_POST['action'] == "delete_status"){
	...
	$query = mysqli_query($connection, "SELECT account_name, author FROM status WHERE id='$statusid' LIMIT 1");
	while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
		$account_name = $row["account_name"];
		$author = $row["author"];
	}
    if ($author == $log_username || $account_name == $log_username) {
		mysqli_query($connection, "DELETE FROM status WHERE osid='$statusid'");
		mysqli_close($connection);
	    echo "delete_ok";
		exit();
	}
...
if (isset($_POST['action']) && $_POST['action'] == "delete_reply"){
	...
	$query = mysqli_query($connection, "SELECT osid, account_name, author FROM status WHERE id='$replyid' LIMIT 1");
	while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
		$osid = $row["osid"];
		$account_name = $row["account_name"];
		$author = $row["author"];
    ...
    if ($author == $log_username || $account_name == $log_username) {
		mysqli_query($connection, "DELETE FROM status WHERE id='$replyid'");
		mysqli_close($connection);
	    echo "delete_ok";
		exit();
	}
